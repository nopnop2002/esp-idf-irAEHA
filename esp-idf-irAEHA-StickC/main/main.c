...
static int readDefineFile(DISPLAY_t *display, size_t maxLine, size_t maxText) {
	...
	while (1){
		...
		display[readLine].ir_customer = strtol(&result[1][0], NULL, 16);
		display[readLine].ir_parity = strtol(&result[2][0], NULL, 16);
		display[readLine].ir_data_num = strtol(&result[3][0], NULL, 10);
		for(int i=0;i<display[readLine].ir_data_num;i++) {
			display[readLine].ir_data[i] = strtol(&result[4+i][0], NULL, 16);
		}
...
void tft(void *pvParameters)
{
	...
	for(int i=0;i<readLine;i++) {
		...
		ESP_LOGI(pcTaskGetTaskName(0), "display[%d].ir_customer=[0x%04x]",i, display[i].ir_customer);
		ESP_LOGI(pcTaskGetTaskName(0), "display[%d].ir_parity=[0x%02x]",i, display[i].ir_parity);
		ESP_LOGI(pcTaskGetTaskName(0), "display[%d].ir_data_num=[%d]",i, display[i].ir_data_num);
		for(int j=0;j<display[i].ir_data_num;j++) {
			ESP_LOGI(pcTaskGetTaskName(0), "display[%d].ir_data[%d]=[0x%02x]",i, j, display[i].ir_data[j]);
		}
	}
	...
	while(1) {
		...
		if (cmdBuf.command == CMD_DOWN) {
			...
			uint16_t customer = display[selected+offset].ir_customer;
			uint8_t parity = display[selected+offset].ir_parity;
			int data_num = display[selected+offset].ir_data_num;
			size_t size = (sizeof(rmt_item32_t) * (AEHA_DATA_ITEM_NUM + (data_num*8)));
			uint8_t data[20];
			memcpy((char *)data, (char*) display[selected+offset].ir_data, data_num);
			//each item represent a cycle of waveform.
			rmt_item32_t* item = (rmt_item32_t*) malloc(size);
			memset((void*) item, 0, size);

			int item_num = aeha_build_items(channel, item, customer, parity, data_num, data);
			ESP_LOGI(TAG, "aeha_build_items item_num=%d", item_num);
...
void tft(void *pvParameters)
{
	...
	while(1) {
		...
		if (cmdBuf.command == CMD_DOWN) {
			...
			//To build a series of waveforms.
			uint16_t customer = display[selected].ir_customer;
			uint8_t parity = display[selected].ir_parity;
			int data_num = display[selected].ir_data_num;
			size_t size = (sizeof(rmt_item32_t) * (AEHA_DATA_ITEM_NUM + (data_num*8)));
			uint8_t data[20];
			memcpy((char *)data, (char*) display[selected].ir_data, data_num);
			//each item represent a cycle of waveform.
			rmt_item32_t* item = (rmt_item32_t*) malloc(size);
			memset((void*) item, 0, size);
